...
class tuflowqgis_menu:
    ...
    def initGui(self):
        ...
        self.insert_TUFLOW_attributes_action = QAction(icon, "Insert TUFLOW Attributes to existing GIS layer",
                                                       self.iface.mainWindow())
        ...
        icon = QIcon(os.path.join(dir, "runner\\tuflow-runner\\icons\\runner", "icons8-exercise.ico"))
        self.launch_runner_action = QAction(icon, "Launch TUFLOW Runner", self.iface.mainWindow())
        # QObject.connect(self.run_tuflow_action, SIGNAL("triggered()"), self.run_tuflow)
        ...
        try:
            ...
            if 'feedback' in kwargs:
                if not kwargs['feedback']:
                    return
            if not no_popup:
                QMessageBox.information(self.iface.mainWindow(), "TUFLOW",
    ...
    def extractRefh2(self):
        ...
        if Refh2Dock is None:
            QMessageBox.critical(self.iface.mainWindow(), "ReFH2",
                                 "A problem occurred loading ReFH2 tool. Please contact support@tuflow.com")
            return
